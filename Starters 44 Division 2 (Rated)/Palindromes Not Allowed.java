...
public class Main {
  ...
  // idea : remove all 2 length and 3 length palindromes
  static void solve() throws Exception{
    long len = l();
    String str = s();
    char ans[] = new char[(int)len];
    
    long boxes = (len+2)/3; // ceil(a/b) = (a+b-1)/b
    long full = 0;
    HashMap<Character,Long> freqMap = new HashMap();
    
    for(int idx=0;idx<len;idx++){
        char ch = str.charAt(idx);
        freqMap.put(ch,freqMap.getOrDefault(ch,0l)+1l);
        long currFreq = freqMap.get(ch);
        
        if(currFreq>boxes){
            sb.append("NO\n");
            return;
        }
        else if(currFreq==boxes) full++;
    }
    
    long rem = len%3; // last box size
    if(rem==0) rem = 3;
    if(full>rem){
        sb.append("NO\n");
        return;
    ...
    for(Pair pair : list){
        ...
        while(freq>0){
            ans[currBox*3+boxSlot] = ch;
            currBox++;
            freq--;
            
            if(currBox*3+boxSlot>=len){
                boxSlot++;
                currBox = 0;
            }
        }
    ...
    // edge case : 3 length palindromes remain 
    for(int idx=2;idx<len;idx++){
        if(ans[idx-2]==ans[idx]){
            swap(ans,idx,idx+1);
        }
    }
  ...
  private static void swap(char ans[],int idx1,int idx2){
      char temp = ans[idx1];
      ans[idx1] = ans[idx2];
      ans[idx2] = temp;
  }
