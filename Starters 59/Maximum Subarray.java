...
public class Main {
  ...
  /*
   We would always want to increase the maximum sum of existing subarray.

   Since we are only interested in finding sum the order of elements does not matter.

   Add all positive elements of b to a, then add all negative elements of b to a.

   Find max sum using kadanes, then reverse order of insertion.

   By favouring ends with positive elements we cover all cases.
  ...
  */
  ...
  static void solve() throws Exception{
    ...
    for(long val : a) arr.add(val);
    for(long val : b){
      if(val>=0) arr.add(val);
    }

    long maxSum = kadanes(arr);
  ...
  private static long kadanes(ArrayList<Long> list){
    long maxSum = Long.MIN_VALUE;
    long currSum = 0;

    for(long val : list){
       if(currSum<0) currSum = val;
       else currSum+=val;
