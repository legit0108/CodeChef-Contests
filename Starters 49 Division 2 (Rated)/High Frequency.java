...
public class Main {
  ...
  // occurs once else we can not reduce maximum frequency
  ...
  static void solve() throws Exception{
    ...
    for(long val : map.keySet()){
      ...
      if(freq>=maxFreq){
        ...
        maxFreq = freq;
        count = 0;
      }else if(freq>secondMaxFreq) secondMaxFreq = freq;
      
      if(freq==maxFreq) count++;
    }
    
    if(count==1) sb.append(Math.max(secondMaxFreq,(maxFreq+1)/2)).append("\n");
    else sb.append(maxFreq).append("\n");
  }
