...
public class Main {
  ...
  // the numbers which have no condition imposed on them
  // can be placed at any index <=n
  // also we maintain a maxheap of all possible numbers 
  // that can be placed at certain index
  // we chose the maximum number to be placed 
  // for an index when traversing from right to left
  // to ensure lexicographically minimum permutation
  ...
  static void solve() throws Exception{
    ...
    for(int num = 1;num<=n;num++){
        list.get(pos[num]).add(num);
    }
    
    for(int idx=n;idx>=1;idx--){
        ArrayList<Integer> numbers = list.get(idx);
        for(int num : numbers) maxHeap.add(num);
