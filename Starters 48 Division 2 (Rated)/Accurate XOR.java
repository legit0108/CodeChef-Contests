...
public class Main {
  ...
  // k subtrees should have odd number of ones
  // so that xor value of those subtrees is 1 
  // and we can sum them up k times
  
  // k subtrees have odd sum
  // n-k subtrees have even sum
  ...
  static void solve() throws Exception{
    ...
    for(int node=1;node<=n;node++) tree[node] = new ArrayList();
  ...
  private static int dfs(int node){
    ...
    if(k>0){
      // this subtree should have odd sum 
      if(subtreeSum%2==0){
          ans[node]=1;
      }else ans[node]=0;
      ...
      // this subtree should have even sum
      if(subtreeSum%2==0){
          ans[node]=0;
      }else ans[node]=1;
    }
    
    return subtreeSum+ans[node];
  }
