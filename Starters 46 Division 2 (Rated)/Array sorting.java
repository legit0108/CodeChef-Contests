...
public class Main {
  ...
  // TC : O(nlog(val)) , val = arr[idx]
  // SC : O(1) assuming array is given

  // idea : maximum value of k 
  // will be gcd of all operations required
  // to place arr[idx] at correct position
  ...
  static void solve() throws Exception{
    ...
    long max =0 ;
    for(int idx = 1;idx<=n;idx++){
        if(arr[idx]!=idx) max = gcd(max,(long)Math.abs(arr[idx]-idx));
    }
