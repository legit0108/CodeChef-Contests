...
public class Main {
  ...
  // xor of consecutive numbers can also be <=1
  // TC: O(n)
  ...
  static void solve() throws Exception{
    ...
    for(int num : map.keySet()){
        ...
        minDels = Math.min(minDels, n-freq);
        
        if((num^(num-1))<=1 && map.containsKey(num-1)) minDels = Math.min(minDels, n-freq-map.get(num-1));
        if((num^(num+1))<=1 && map.containsKey(num+1)) minDels = Math.min(minDels, n-freq-map.get(num+1));
    }
    
    sb.append(minDels).append("\n");
  }
