...
public class Main {
  ...
  /*
    ...
    Si -> set of all strings having exactly i bits flipped,
    answer won't depend on initial string,
    just need to look at whether we can flip some bits using some operations,
    k = 1 -> all strings of length n can be generated -> answer 2^n,
    k = n -> can generate 2 distinct strings,
    flipping i bits -> nCi selections,
    if k is even,
    we can do even flips,
    ans = nC0 + nC2 + nC4 + ..... = 2^(n-1),
    every operation flip 2 bits,
    else if k is odd,
    by performing a set of operations you can flip one bit,
    can generate all 2^n strings
  ...
  */
  ...
  static void solve() throws Exception{
    ...
    if(k==1) sb.append(p(2,n)).append("\n");
    else if(k==n) sb.append(2).append("\n");
    else if(k%2==0) sb.append(p(2,n-1)).append("\n");
    else if(k%2!=0) sb.append(p(2,n)).append("\n");
  }
