...
public class Main {
  ...
  static void solve() throws Exception{
   ...
   for(int bit = n-1;bit>=0;bit--){
       long mask = 1l<<bit;
       if((((a&mask)==0l&&(b&mask)==0l))){
           ans|=mask; // if bit is unset in both then its always optimal to set it in ans
       }
       else if(((a&mask)==0l)||((b&mask)==0l)){ // set bit in ans if and only if better product obtained
           long productSoFar = (a^ans)*(b^ans);
           long tempMask = ans|mask;
           long currProduct = (a^tempMask)*(b^tempMask);
           if(currProduct>productSoFar) ans|=mask;
       }
   }
