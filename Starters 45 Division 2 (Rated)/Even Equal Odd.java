...
public class Main {
  ...
  // TC : O(nlogn + nlog(max(arr[idx])))
  // SC : O(1)
  
  static void solve() throws Exception{
   int n = i();
   long arr[] = readArray((long)2*n);
   long countEven = 0;
   long countOdd = 0;
   
   for(int idx =0;idx<2*n;idx++){
       if(arr[idx]%2==0) countEven++;
       else countOdd++;
   }
   
   if(countEven==countOdd) sb.append("0\n"); // no ops required
   else if(countOdd>countEven) sb.append((countOdd-countEven)/2l).append("\n"); // convert odd to even by multiplying with 2
   else{
       // convert even to odd
       // convert those even elements
       // that have minimal power of 2 in their prime factorization to minimize ops 
       ...
       long diff = (countEven-countOdd)/2;
       long ans = 0;
       int size = list.size();
       
       for(int idx =0;idx<size&&diff>0;idx++){
           diff--;
           ans+=list.get(idx);
       }
