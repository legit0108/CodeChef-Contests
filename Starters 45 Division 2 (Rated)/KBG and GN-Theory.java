...
public class Main {
  ...
  // idea : build adjacency list run dijkstra from node 1
  // minimum distance between u and v = minimum distance(1->u) + minimum distance(1->v) - 2*minimum distance(1->gcd(u,v))
  ...
  static void solve() throws Exception{
    ...
    for(int node1=1;node1<=n;node1++){
        for(int node2=2*node1;node2<=n;node2+=node1){
            graph[node1].add(new int[]{node2,node2/node1});
            graph[node2].add(new int[]{node1,node2/node1});
        }
    }
    
    dijkstra(1,n,graph,minDist);
    
    while(q-->0){
        int u = i();
        int v = i();
        
        long dist = minDist[u]+minDist[v]-2*minDist[(int)gcd(u,v)];
        sb.append(dist).append("\n");
    }
  }
  
  private static void dijkstra(int src,int nodes,ArrayList<int[]> graph[],int minDist[]){
      PriorityQueue<int[]> minHeap = new PriorityQueue<int[]>((arr1,arr2)->Integer.compare(arr1[1],arr2[1]));
      ...
      while(minHeap.size()>0){
          ...
          for(int nextArr[]: graph[node]){
              int child = nextArr[0];
              int edgeWt = nextArr[1];
              
              if(!visited[child]) minHeap.add(new int[]{child,cost+edgeWt});
          }
      }
  }
