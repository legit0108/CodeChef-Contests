...
public class Main {
  ...
  // idea : if adjacent elements have same parity
  // then arr[idx] = 0 else arr[idx] = 1
  // simulate to find parity of each element
  // at the end check if parities of last element
  // and first element match according to whats given
  // if they do then ans is yes else ans is no 
  
  static void solve() throws Exception{
   int n = i();
   long arr[] = readArray((long)n);
   long initParity = 0;
   long parity = initParity;
   
   for(int idx =0;idx<n-1;idx++){
       if(arr[idx]==1){ //next elem diff parity
           parity^=1;
       }
   }
   
   if(initParity==parity){
      if(arr[n-1]==1) sb.append("NO\n");
      else sb.append("YES\n");
   }else{
       if(arr[n-1]==1) sb.append("YES\n");
       else sb.append("NO\n");
   }
  }
